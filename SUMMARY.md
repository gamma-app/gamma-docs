# Table of contents

* [Gamma Developer Docs](README.md)

## Get Started

* [Explore the API](overview/understanding-the-api-options.md)
* [Review access and pricing](overview/access-and-pricing.md)

## Generations

* [POST /generations](endpoints/create-generation.md)
* [POST /generations/from-template](endpoints/create-from-template.md)
* [GET /generations/{id}](endpoints/get-generation-status.md)

## Workspace

* [GET /themes](endpoints/list-themes.md)
* [GET /folders](endpoints/list-folders.md)

## Guides

* [Generate from text](overview/generate-api-parameters-explained.md)
* [Generate from template](overview/create-from-template-api-parameters-explained.md)
* [Poll for results](overview/async-patterns-and-polling.md)
* [Use themes and folders](overview/list-themes-and-list-folders-apis-explained.md)
* [Format headers and footers](overview/header-and-footer-formatting.md)
* [Optimize image URLs](overview/image-url-best-practices.md)
* [Add charts and structured content](overview/charts-and-structured-content.md)

## MCP & Connectors

* [Set up the MCP server](overview/gamma-mcp-server.md)
* [Connect integrations](overview/connectors-and-integrations.md)

## Reference

* [Changelog](overview/changelog.md)
* [Image models](accepted-values/image-model-accepted-values.md)
* [Output languages](accepted-values/output-language-accepted-values.md)
* [Error codes](errors-and-warnings/error-codes.md)
* [Warnings](errors-and-warnings/warnings.md)
* [API scope and limits](overview/common-feature-requests.md)
* [Get help](overview/get-help.md)
