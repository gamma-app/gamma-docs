...
The API uses an asynchronous polling model. After creating a generation, poll `GET /generations/{id}` until the status is `completed` or `failed`. See [Async Patterns and Polling](async-patterns-and-polling.md) for implementation patterns across platforms.
